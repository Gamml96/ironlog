...
const PostCard = memo(({ 
  ...
  groupId: string;
  creatorId?: string;
  onLike: (p: GroupPost) => void;
  onDelete: (p: GroupPost) => void;
  onEdit: (p: GroupPost) => void;
  editingPostId: string | null;
  editContent: string;
  setEditContent: (v: string) => void;
  onSaveEdit: (p: GroupPost) => void;
  onCancelEdit: () => void;
  openCommentsPostId: string | null;
  setOpenCommentsPostId: (id: string | null) => void;
  newCommentText: string;
  setNewCommentText: (v: string) => void;
  onAddComment: (p: GroupPost, text: string) => void;
  deletingCommentId: {postId: string, index: number} | null;
  ...
  const handleDeleteComment = async (commentIndex: number) => {
    ...
    if (isConfirming) {
      try {
        const postRef = doc(db, 'groups', groupId, 'feed', post.id);
  ...
  return (
    <Card className="p-0 overflow-hidden border-white/5 relative group/post hover:border-brand-primary/20 transition-all duration-300">
      <div className="p-5 space-y-4">
                ...
                <div className="flex gap-1.5">
                  ...
                  <button 
                    ...
                    className={`flex items-center gap-2 p-2.5 transition-all duration-300 rounded-xl active:scale-95 ${
                      isDeleting ? 'bg-red-500 text-white px-4' : 'bg-white/5 text-muted hover:text-red-500'
                    }`}
                  >
                    {isDeleting ? (
                      <>
        ...
        {isEditing ? (
          <div className="space-y-3 bg-black/20 p-4 rounded-2xl border border-brand-primary/20">
            ...
            <div className="flex justify-end gap-2">
               <button onClick={onCancelEdit} className="text-[10px] font-black uppercase text-muted py-2 px-4 hover:text-white transition-colors">Cancelar</button>
               <button onClick={() => onSaveEdit(post)} className="text-[10px] font-black uppercase bg-brand-primary text-black py-2 px-6 rounded-xl hover:scale-105 active:scale-95 transition-all">Salvar</button>
            </div>
          </div>
        ) : (
        ...
        {post.type === 'workout' && post.workoutData && (
          <div className="bg-gradient-to-br from-brand-primary/10 to-brand-primary/5 border border-brand-primary/20 rounded-3xl p-5 space-y-4 relative overflow-hidden group/workout shadow-lg">
            ...
            {post.content && post.type === 'workout' && !isEditing && (
        ...
        <div className="flex items-center gap-6 pt-3 px-1">
          ...
          <button 
            onClick={() => setOpenCommentsPostId(areCommentsOpen ? null : post.id)}
            className={`flex items-center gap-2 transition-all ${areCommentsOpen ? 'text-brand-primary' : 'text-muted hover:text-white'}`}
          >
            <div className={`p-2 rounded-full transition-colors ${areCommentsOpen ? 'bg-brand-primary/10' : 'bg-white/5 group-hover/post:bg-white/10'}`}>
...
export function GroupFeedView({ group, currentUser }: GroupFeedViewProps) {
  ...
  const handleLike = useCallback(async (post: GroupPost) => {
  ...
  const handleAddComment = useCallback(async (post: GroupPost, text: string) => {
    if (!text.trim()) return;
    try {
      const postRef = doc(db, 'groups', group.id, 'feed', post.id);
      const newComment = {
  ...
  const handleDeletePost = useCallback(async (post: GroupPost) => {
  ...
  }, [group.id, deletingPostId]);

  const startEdit = useCallback((post: GroupPost) => {
    setEditingPostId(post.id);
    setEditContent(post.content || '');
  }, []);

  const saveEdit = useCallback(async (post: GroupPost) => {
    try {
